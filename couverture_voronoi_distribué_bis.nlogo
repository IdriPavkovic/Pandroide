globals [
  ...
  nombre_patch_decouvert  ;; nombre de patches non protégés de la zone actuelle
  nombre_patch  ;; nombre de patch de la zone actuelle
]
...
to setup
  ...
  set nombre_patch (count patches with [zone = zone-actuelle])
  write "nombre de patchs :"
  show nombre_patch
...
to simuler
  ifelse zone-actuelle < 4 [
    write "pourcentage non couvert : "
    show ((nombre_patch_decouvert / nombre_patch) * 100)
    if attaque  [missile force_impacte]   ;; procédure lancant un missible de force_impacte (disque blanc sur environnement)
    wait 1       ;; pause de 1 seconde pour voir l'impacte
    define_environment 35 75 120
    voronoi
    positionner
    if repli [change_zone]
  ]
  [set zone-actuelle 3
   voronoi
   write "GAME OVER"
...
;;; alors
to positionner
  set nombre_patch_decouvert 0
  ask protectors [
    set xcor mean [pxcor] of patches with [pcolor = [color] of myself]
    set ycor mean [pycor] of patches with [pcolor = [color] of myself]
    let points ((patches with [pcolor = [color] of myself]) with [distance myself > r-protection]); patches de la zone non protégés
    ask points [
      ...
      if d > r-protection [ set nombre_patch_decouvert (nombre_patch_decouvert + 1)]   ;on compte le nombre de patches de la région non protégés
    ]
  ]
  write "nombre patch :"
  show nombre_patch
  write "nombre patchs non couvert :"
  show nombre_patch_decouvert
  if ((nombre_patch_decouvert / nombre_patch) * 100) > faiblesse [set repli true] ; repli si le pourcentage de patch est supérieur à faiblesse (curseur)
end
